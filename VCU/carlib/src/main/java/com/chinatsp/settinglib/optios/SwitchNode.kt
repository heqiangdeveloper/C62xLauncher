...
//    DRIVE_WHEEL_AUTO_HEAT(Norm(on = 1, off = 0, uon = 1, uoff = 2), set = CarCabinManager.ID_SWS_HEAT_SWT, get = CarCabinManager.ID_SWH_STATUS),
     ...
     */
//    DRIVE_SAFE_FORTIFY_SOUND(Norm(don = 2, doff = 1, def = false), set = CarCabinManager.ID_LOCK_SUCCESS_SOUND_SET, get = CarCabinManager.ID_LOCK_SUCCESS_SOUND_STATUE),
...
//    AC_AUTO_DEMIST(Norm(on = 1, off = 0, uon = 1, uoff = 2, def = false), SignalOrigin.HVAC_SIGNAL, set = CarHvacManager.ID_HVAC_AVN_KEY_DEFROST, get = CarHvacManager.ID_HAVC_AC_DIS_DEFROST),
    ...
    AUDIO_SOUND_LOUDNESS(
        ...
        set = Norm(signal = CarCabinManager.ID_HUM_LOUD_SW),
        default = true
    ),
    ...
    /**
     * 车门车窗-外后视镜-后视镜自动折叠
     * set -> AVM EVM View Switch电子外后视冗余影像系统工作开关 0x0: Inactive; 0x1: On; 0x2: OFF; 0x3: Invalid
     *
     */
    AS_BACK_MIRROR_FOLD(
        get = Norm(on = 0x1, off = 0x2),
        set = Norm(on = 0x1, off = 0x2, signal = CarCabinManager.ID_APA_AVM_EVM_VIEW_SWT),
        default = true
    ),
    //----------------车门与车窗 结束--------------
     ...
     */
    ADAS_SLA(
        get = Norm(on = 0x2, off = 0x1, signal = CarCabinManager.ID_HUM_ICM_SPEEDLIMIT_STATS),
        set = Norm(on = 0x2, off = 0x1, signal = CarCabinManager.ID_HUM_ICM_SPEEDLIMIT_STATS),
        default = true
    ),
     ...
     * 驾驶辅助--侧后辅助--开门预警 DOW
     * get ->
        开门预警功能状态信号(高配的APA发送该信号，低配的AVM不发送)
    ...
    fun isOn(value: Int) = get.on == value
